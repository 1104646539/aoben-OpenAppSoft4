        ...
        checkService.LogIn(Global.URL_LOGIN, loginBean).subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread()).subscribe(new Subscriber<BaseResult<LoginResultBean>>() {
                    ...
                    public void onCompleted() {
                        ...
                        editor.putString("user", loginBean.getName()); //存入String型数据
                    ...
                    @Override
                    public void onError(Throwable e) {
                        Toast.makeText(context, "登录失败,网络连接错误", Toast.LENGTH_SHORT).show();
...
//                }
                    ...
                    @Override
                    public void onNext(BaseResult<LoginResultBean> result) {
                        if (result.code == 200) {
                            if (checkInterface != null) {
                                checkInterface.logInSuccess(result.data);
                            }
                        } else {
                            if (checkInterface != null) {
                                checkInterface.logInFailed(result.message);
                            }
                        }
                    }
                });
