...
public class TSCheckActivity extends BaseActivity implements View.OnClickListener, CheckPresenter.CheckInterface, StatusDialog.OnProgressStatusChange, AdapterView.OnItemSelectedListener {
    String data = "";
    String productId = "";
    String objectId = "";
    /**
...
//    private Spinner sp_yplx;
    ...
    //图片搜索
    @GT.Annotations.GT_View(R.id.iv_search_check)//搜索
    private ImageView ivSearchCheck;
    //被检企业
    @GT.Annotations.GT_View(R.id.tv_CheckedEnterprise)//被检企业
    private TextView tvCheckedEnterprise;
...
//    private Spinner sp_yplx1;
    //样本类型
    @GT.Annotations.GT_View(R.id.tv_yplx_normal)
    private TextView tv_yplx_normal;
    @GT.Annotations.GT_Collection.GT_Map
    ...
    private void scanParameterSuccess(final String temp) {
        ...
        if (temp == null || temp.contains("OK2")) {
            TSCheckActivity.this.runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(TSCheckActivity.this, "当前网络不佳，请从新扫描。", Toast.LENGTH_SHORT).show();
                }
            });
            return;
        }
...
//                Toast.makeText(CheckActivity.this, "scanParameterSuccess=" + temp, Toast.LENGTH_SHORT).show();
                ...
                if (statusDialog != null) {
                    ...
                    checkPresenter.GetCardQRInfo(new GetQRInfoBean(cardCode), 1, TSCheckActivity.this);
...
//        statusDialog.setStatus(StatusDialog.STATUS_END_SCAN_QRCODE_CARD);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_tscheck);
        //软键盘不会遮挡了
        ...
        // 注册广播
        locationReceiver = new TSCheckActivity.LocationReceiver();
...
//        sp_yplx_normal = findViewById(R.id.sp_yplx_normal);
        tv_yplx_normal = findViewById(R.id.tv_yplx_normal);
        // 获取参数
        ...
        WaitDialog.show("数据获取中");
        //样本类型
        tv_yplx_normal.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                List<String> list = new ArrayList();
                for (int i = 0; i < jsType.size(); i++) {
                    list.add(jsType.getJsonMap(i).getString("sampleName"));
                }
...
//         判断是否是混合输入状态
        ...
        mode_exchange.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent inent = new Intent(TSCheckActivity.this, CheckActivity.class);
                inent.putExtra("source", "2");// 定性
                startActivity(inent);

                progressDialog = new ProgressDialog(TSCheckActivity.this);
        ...
        //进出卡
        findViewById(R.id.btn_outCard).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
...
//		}
        ...
        company_adater = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, company_list);//simple_spinner_item
        persion_adapter = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, persion_list);
        shiji_adapter = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, shiji_list);
        sample_adapter = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, sample_list);
        project_adapter = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, project_list);
        type_adapter = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, type_list);
        sampleUnit_adapter = new ArrayAdapter<String>(TSCheckActivity.this, R.layout.item_simple_spiner, sampleUnit_list);
...
//                statusDialog.show();
    ...
    TSCheckActivity.QRCodeBroadcastReceiver qrCodeBroadcastReceiver;

    private void registerReceiver() {
        //二维码广播接收机
        qrCodeBroadcastReceiver = new TSCheckActivity.QRCodeBroadcastReceiver();
        IntentFilter intentFilter = new IntentFilter();
...
//                                ArrayAdapter<String> adapter = new ArrayAdapter<>(CheckActivityByMen.this, R.layout.simple_list_item_choice_shoudong, array);
                                        ArrayAdapter<String> adapter = new ArrayAdapter<>(TSCheckActivity.this, android.R.layout.simple_spinner_dropdown_item, array);
...
//                                adapter = new ArrayAdapter<>(CheckActivityByMen.this, R.layout.simple_list_item_choice_shoudong, array);
                                        adapter = new ArrayAdapter<>(TSCheckActivity.this, android.R.layout.simple_spinner_dropdown_item, array);
...
//                    Timber.i(data);
                    GT.toast(TSCheckActivity.this, linkageJsonRootBean.getErrMsg());
                }
...
//                    Timber.i(data);
                    GT.toast(TSCheckActivity.this, sampleSourceJsonRootBean.getErrMsg());
                }
            }
...
//        sp_yplx1 = findViewById(R.id.sp_yplx1);
        ...
        btn_paint1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                showEditDialog(v);
            }
        ...
        btn_location.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!GT.Network.isInternet(TSCheckActivity.this)) {
                    new AlertDialog.Builder(TSCheckActivity.this)
...
//        btn_Imm_Check.requestFocusFromTouch();
    ...
    private void ClickUpload() {
        if (resultModel == null) {
            APPUtils.showToast(TSCheckActivity.this, "请先检测");
            return;
        }
        if (uploadingFlag) {
...
//                        ToolUtils.hiddenHUD();
                        ...
                        Toast.makeText(TSCheckActivity.this, str, Toast.LENGTH_LONG).show();
                    }
...
//			resultModel.shiji = shiji_model.getName();
            ...
            resultModel.sample_type = tv_yplx_normal.getText().toString();
...
//        detectionResultBean.setUploadStatus("已上传");
        ...
        detectionResultBean.setYplbId(yplbId + ""); // 样品
        detectionResultBean.setSampleId(jsTypeId);//样品总分类Id
...
//        }
        ...
        if (isTest) {
            APPUtils.showToast(TSCheckActivity.this, "二维码扫描中，请稍后...");
            return;
        }
...
//            }
            ...
            APPUtils.showToast(TSCheckActivity.this, str);
            return;
        }
        ...
        APPUtils.showToast(TSCheckActivity.this, "扫描二维码中，请稍后...");
        ...
        GT.Thread.runJava(new Runnable() {
            ...
            public void run() {
                ...
                if (isStartTest == 0) {
                    APPUtils.showToast(TSCheckActivity.this, "1请检查金标卡是否插到位与网络是否正常连接：" + isStartTest);
                    isTest = false;
...
//                    CardOutOrIn();
                    ...
                    APPUtils.showToast(TSCheckActivity.this, "该二维码数据有误");
...
//                    CardOutOrIn();
                ...
                TSCheckActivity.this.runOnUiThread(new Runnable() {
                    public void run() {
                        if (tv_check_company.getText() == null || tv_check_company.getText().toString().isEmpty()) {
                            APPUtils.showToast(TSCheckActivity.this, "请先选择检测单位");
                        ...
                        if (tv_check_persion.getText() == null || tv_check_persion.getText().toString().isEmpty()) {
                            APPUtils.showToast(TSCheckActivity.this, "请先选择检验员");
                        ...
                        if (tv_check_sample.getText() == null || tv_check_sample.getText().toString().isEmpty()) {
                            APPUtils.showToast(TSCheckActivity.this, "请先选样品名称");
                            tv_scanTime.setText("请先选样品名称");
                            isTest = false;
                            return;
                        }
                        if (tv_yplx_normal.getText() == null || tv_yplx_normal.getText().toString().isEmpty()) {
                            APPUtils.showToast(TSCheckActivity.this, "请先选样品类型");
                        ...
                        if (et_yplydz.getText() == null || et_yplydz.getText().toString().isEmpty()) {
                            APPUtils.showToast(TSCheckActivity.this, "请输入样品详细地址");
                        ...
                        if (new Integer("0").equals(selectedProject.ScanEnd)
                            ...
                            APPUtils.showToast(TSCheckActivity.this, "该检测项目参数不完整或不正确，请转至项目管理并重新编辑");
                            isTest = false;
                            return;
                        }
                    }
    ...
    private void SendData(String message) {
        ...
        ToolUtils.showHUD(TSCheckActivity.this, "请稍等...");

        timer = new Timer();
        if (myTimerTask != null) {
            myTimerTask.cancel();
        }
        myTimerTask = new TSCheckActivity.MyTimerTask();
    ...
    class MyTimerTask extends TimerTask {
        ...
        public void run() {
            if (readTimeOutCount++ > 3) {
                ...
                APPUtils.showToast(TSCheckActivity.this, "接收数据失败");
                    ...
                    if (i == 1) {
                        ...
                        if (retSize > 0) {
                            ...
                            if (data.length != 2) {  //如果数据格式错误
                                APPUtils.showToast(TSCheckActivity.this, "数据格式错误，接收数据失败");
                                handler.sendEmptyMessage(100);
                                return;
                            }
    ...
    private void CalcAndShowTestResult(String[] data) {
        ...
        try {
            ...
            APPUtils.showToast(TSCheckActivity.this, "数据格式错误，接收数据失败");//几乎不可能异常，除非下位机出现问题
            handler.sendEmptyMessage(100);//关闭定时器
        ...
        try {
            ...
            APPUtils.showToast(TSCheckActivity.this, "计算错误");
            e.printStackTrace();
    ...
    private void uploadResult() {
        ...
        if (jsSubTypePosition != -1) {
            if (jsSubType != null) {
                productId = jsSubProductId;
                objectId = jsSubTypeId;
            }
        ...
        if ("农药残留单项精准分析仪".equals(com.open.soft.openappsoft.util.InterfaceURL.oneModule)) {
            ...
            data = "[{\"QRCode\":\"" + QRCode22.replaceAll("\\s*", "") + "\",\n" +
                    ...
                    "{\"Title\":\"样品总分类Id\",\"Id\":\"sampleClassId\",\"Value\":\"" + jsTypeId + "\"},\n" +
                ...
                if (uploadData1 != null) {
                    ...
                    runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            Toast.makeText(TSCheckActivity.this, errMsg, Toast.LENGTH_LONG).show();
                            if ("0".equals(errCode)) {
                                if (detectionResultBean != null) {
                                    detectionResultBean.setUploadStatus("已上传");
                                    MainActivity.hibernate.save(detectionResultBean);
...
//                                Timber.i("上传-------------------");
                                GT.toast(TSCheckActivity.this, errMsg);
                            }
...
//        super.onBackPressed();
    ...
    private void timeCheck() {
        ...
        taskTime = new TSCheckActivity.MytaskTime();
        timer = new Timer();
    ...
    private void ShowSampleTimeDialog() {
        ...
        new DateTimePickerDialog(TSCheckActivity.this, 0,
...
//                Timber.i("body=" + body);
                ...
                if ("0".equals(sampleTypeJsonRootBean.getErrCode())) {
                    JsonMap data = JsonMap.parse(body);
...
//                    Timber.i(data);
                    GT.toast(TSCheckActivity.this, data);
                }
            }
                ...
                GT.Thread.runAndroid(TSCheckActivity.this, new Runnable() {
                    ...
                    public void run() {
                        if (!"".equals(LocationAddress) && !"".equals(LocationX) && !"".equals(LocationY)) {
                            if (isLocation) {
                                APPUtils.showToast(TSCheckActivity.this, "定位成功");
                            }
                            isLocation = false;
                            et_jcxxdz.setText(LocationAddress);
                        } else {
                            if (isLocation) {
                                APPUtils.showToast(TSCheckActivity.this, "定位失败");
                            }
                            isLocation = false;
                            et_jcxxdz.setText("");
    ...
    public void ClickDraw1(View v) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                if (imageData == null) {
                    APPUtils.showToast(TSCheckActivity.this, "图像数据获取失败");
                    return;
                }
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case 100:
                    ...
                    (TSCheckActivity.this).onActivityResult(TSCheckActivity.REQUEST_CODE, Activity.RESULT_OK, intent);
                    ...
                    try {
                        ...
                        APPUtils.showToast(TSCheckActivity.this, "接收数据错误");
                    } catch (Exception e) {
                        e.printStackTrace();
                    } finally {
//                        ToolUtils.hiddenHUD();
...
//			mChart.animateXY(2000, 2000);
    ...
    public void savaImage() {
        ...
        // 保存图片
        GT.ApplicationUtils.saveImage(TSCheckActivity.this, mChart, appSavePath, "曲线.png");
    ...
    public void showEditDialog(View view) {
        ...
        if (new_ints == null) {
            GT.toast(TSCheckActivity.this, "还没有检测数据，无法查看绘图");
            return;
        }
...
//        DialogFragmentPaint.newInstance(bundle).show(CheckPaintActivity.this.getSupportFragmentManager(), "");
        DialogFragmentPaint.newInstance(bundle).show(TSCheckActivity.this.getSupportFragmentManager(), "");
        ...
        new GT.HttpUtil().postRequest(InterfaceURL.BASE_URL + "/CommonManual/GetProductList", map2, new GT.HttpUtil.OnLoadDataListener() {
            ...
            public void onSuccess(String response, Object o) {
                ...
                if (data.getString("ErrCode").equals("0")) {
                    ...
                    Toast.makeText(TSCheckActivity.this, data.getString("ErrMsg"), Toast.LENGTH_SHORT).show();
                }
            }
