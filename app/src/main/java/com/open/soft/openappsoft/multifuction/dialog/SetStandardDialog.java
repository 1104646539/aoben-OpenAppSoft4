 ...
 * 设置检测依据
 */
public class SetStandardDialog extends Dialog {
    ...
    public SetStandardDialog(@NonNull Context context) {
        super(context);
    ...
    private void init() {
        contentView = LayoutInflater.from(context).inflate(R.layout.dialog_set_standard_layout, null, false);
        ...
        ll_show = (LinearLayout) contentView.findViewById(R.id.ll_show);

//        getWindow().setBackgroundDrawableResource(android.R.color.transparent);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
    }

    public void showDialog(String title, String msg,final int type) {
        this.type = type;
        if (isShowing()) {
            dismiss();
        }
            ...
            public void onClick(View v) {
                if (onConfirmListener != null) {
                    if (et_content.getText().toString().trim().isEmpty()) {
                        Toast.makeText(context, "请输入检测依据", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    dismiss();
