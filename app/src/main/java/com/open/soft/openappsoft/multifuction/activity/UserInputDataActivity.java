...
import com.open.soft.openappsoft.multifuction.interfaces.ISaveCallBack;
...
public abstract class UserInputDataActivity<T extends UserInputModel> extends Activity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.btn_clear_data) {
            ...
            deleteData();
        } else if (i == R.id.btn_edit_data) {
            if (adapter.getSelectedCount() == 0) {
                APPUtils.showToast(act, "请先选中数据");
                return;
            }
            getDialog(adapter.getSelectedList().get(0)).setCallBack(new ISaveCallBack<T>() {
                @Override
            ...
            getDialog(null).setCallBack(new ISaveCallBack<T>() {
                @Override
                public boolean save(T t) {
                    if (dataIsExist(t)) {
